...
async function sendTelegramMessage(
  ...
  const chatId = String(replyChatId || TELEGRAM_CHANNEL_ID);
  ...
  if (replyId !== undefined && Number.isFinite(replyId) && replyId > 0) {
  ...
  try {
    ...
    return {
      success: false,
      error: retryResult?.description || result?.description || `Telegram HTTP ${response.status}`,
    };
  } catch (error) {
    return {
      success: false,
...
function formatSignalMessage(signal: Signal): string {
  const rawType = String(
    signal.type || (signal as any).action || (signal as any).direction || ""
  ).toUpperCase();

  const directionEmoji = rawType === "BUY" ? "💹" : rawType === "SELL" ? "🔻" : "";

  let message = `New Signal 🚦\n`;
  message += `━━━━━━━━━━━━━━━━━\n`;
  message += `📊 ${escapeHtml(signal.pair)}  ${escapeHtml(rawType)} ${directionEmoji}\n`;
  message += `                                      (Open)\n\n`;
  message += `💰 Entry: ${escapeHtml(signal.entry)}\n`;
  message += `🎯 TP1: ${escapeHtml(signal.tp1)}\n`;

  if (signal.tp2) message += `🎯 TP2: ${escapeHtml(signal.tp2)}\n`;
...
/* =========================================================
   UPDATED PROFIT NOTES & MESSAGES FOR TELEGRAM
========================================================= */
...
> = {
  tp1_hit: {
    headline: "BOOM 💥 ! TP 1 Hit! 🚀 First Profit Secured ✅",
    banner: "🎯",
  },

  tp2_hit: {
    headline: "BOOM! TP 2 Hit Secured! 💰 Enjoy Profit 💵 ✅✅",
    banner: "🎯",
  },

  tp3_hit: {
    headline: "AMAZING! TP 3 Hit Final target Hit 🎉 Maximum Profit Secured 💵✅",
    banner: "🏆",
  },

  tp4_hit: {
    headline: "AMAZING! TP 4 Hit Final target Hit 🎉 Maximum Profit Secured 💵✅",
    banner: "🏆",
  },
...
function formatUpdateMessage(
  ...
  let meta = UPDATE_META[updateType || ""] || {
    headline: "🔔 SIGNAL UPDATE",
    banner: "🔔",
  };

  /*
   * Breakeven Updates after TP1 or TP2 Hit
   */
  if (isSl && signal.tp2_hit) {
    meta = {
      headline: "Signal Closed at Breakeven after TP2 Hit ✅✅",
      banner: "⚪",
    };
  } else if (isSl && signal.tp1_hit) {
    meta = {
      headline: "Signal Closed at Breakeven after TP1 Hit ✅",
      banner: "⚪",
    };
  }

  return `${meta.banner} <b>${escapeHtml(meta.headline)}</b> ${meta.banner}`;
}
...
function formatIdeaMessage(idea: Idea): string {
  const category = idea.category || idea.main_category || "Market Ideas";
  const pair = idea.pair || idea.symbol || "";
  const direction = idea.direction || idea.type || "";

  let message = `💡 <b>MARKET IDEA</b> 💡\n`;
  message += `━━━━━━━━━━━━━━━\n\n`;
  message += `📂 <b>Category:</b> ${escapeHtml(category)}\n`;

  if (pair) message += `📊 <b>Pair:</b> ${escapeHtml(pair)}\n`;

  if (direction) {
    const upperDirection = String(direction).toUpperCase();
    const directionEmoji = upperDirection === "BUY" ? "💹" : upperDirection === "SELL" ? "🔻" : "📈";
    message += `${directionEmoji} <b>Direction:</b> ${escapeHtml(upperDirection)}\n`;
  }

  if (idea.timeframe) message += `⏱ <b>Timeframe:</b> ${escapeHtml(idea.timeframe)}\n`;
  ...
  const analysis = idea.analysis || idea.analysis_reason || "";
  if (analysis) {
    message += `\n📝 <b>Chart Analysis</b>\n`;
    message += `<i>${escapeHtml(analysis)}</i>\n`;
  }
...
function isIdeaAction(action: unknown): boolean {
  ...
  return ["new_idea", "idea", "market_idea", "auto_idea", "auto_ideas"].includes(value);
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { signal, idea, action, update_type, session_type, stats, force } = body;
    const phase = getMarketPhase();

    const resolvedAction =
      action ||
      (signal?.action ? "new_signal" : signal?.direction ? "new_signal" : idea ? "new_idea" : null);
    ...
    if (resolvedAction === "new_signal") {
      ...
      if (!force && !isAllowedForChannel(signal.pair, phase)) {
        ...
        const existingChatId = signal.telegram_chat_id || TELEGRAM_CHANNEL_ID;

        if (
          existingMessageId !== undefined &&
          existingMessageId !== null &&
          String(existingMessageId).trim() !== "" &&
          Number(existingMessageId) > 0
        ) {
          result = await sendTelegramMessage(message, Number(existingMessageId), String(existingChatId));
          if (!result.success) {
            result = await sendTelegramMessage(message);
          }
        } else {
      ...
      if (!session_type) {
        return new Response(
          JSON.stringify({ success: false, error: "session_type is required" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      const message = formatSessionMessage(session_type, stats);
      result = await sendTelegramMessage(message);
    } else if (isIdeaAction(resolvedAction) || idea) {
      if (!idea) {
        return new Response(
          JSON.stringify({ success: false, error: "Idea data required" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      const message = formatIdeaMessage(idea);
      const imageUrl = idea.image_url || idea.chart_url || null;
      ...
      return new Response(
        JSON.stringify({
          ...
          hint: "Use new_signal, update, or new_idea",
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
