...
// MARKET HOURS
// ---------------------------------------------------------
// IMPORTANT: This checks the REAL MARKET schedule, not the
// signal row `status`. A pair is eligible for generation only
// when its market is open. Existing OPEN/CLOSED signal status
// is checked separately by evaluatePair().
//
// Times below are UTC. Crypto and Deriv synthetic markets are
// treated as 24/7. Traditional FX/metals/indices are closed
// over the weekend. The Sunday/Friday boundaries also prevent
// generation while those markets are closed around the weekly
// rollover.
// =========================================================

function isTraditionalMarketOpen(pair: string, now = new Date()): boolean {
  const day = now.getUTCDay(); // 0 = Sunday, 6 = Saturday
  const minutes = now.getUTCHours() * 60 + now.getUTCMinutes();

  // Weekly close/open window for FX and most CFD/metals symbols.
  // Sunday 22:00 UTC -> Friday 22:00 UTC.
  if (day === 0) return minutes >= 22 * 60;
  if (day === 6) return false;
  if (day === 5) return minutes < 22 * 60;

  // Monday-Thursday are open. Individual instruments can still
  // have broker-specific intraday breaks; the live-price check
  // below remains a second safety check.
  return true;
}

function isMarketOpen(pair: string, now = new Date()): boolean {
  // 24/7 markets in this application.
  ...
  if (cryptoPairs.has(pair) || derivPairs.has(pair)) {
    return true;
  }

  // Gold, silver, FX and indices are traditional markets.
  return isTraditionalMarketOpen(pair, now);
}

function filterOpenMarketPairs<T extends { pair: string }>(pairs: T[]): T[] {
  const now = new Date();
  return pairs.filter((item) => {
    const open = isMarketOpen(item.pair, now);
    if (!open) {
      console.log(`Skipping ${item.pair}: market CLOSED`);
    }
    return open;
  });
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // =====================================================
    // MARKET OPEN CHECK — BEFORE RANDOM SELECTION
    // -----------------------------------------------------
    // Do NOT pick from all pairs first and then reject a closed
    // pair. That would make the whole cron run generate nothing
    // even when another market is open. Filter closed markets
    // first, then apply the existing weighted random selection.
    // =====================================================

    const openWeighted = filterOpenMarketPairs(weighted);

    if (openWeighted.length === 0) {
      return new Response(
        JSON.stringify({
          success: true,
          generated: false,
          reason: "No markets are currently open",
        }),
        {
          headers: {
            ...corsHeaders,
    ...
    const selected = pick(openWeighted);
