...
BEGIN
  ...
  WITH daily_stats AS (
    SELECT 
      ...
      SUM(COALESCE(
        (regexp_match(s.pips_result, '([+-]?\d+\.?\d*)'))[1]::numeric,
        0
      )) as pips
    FROM signals s
  ...
  SELECT 
    ...
    CASE 
      WHEN SUM(CASE WHEN s.tp1_hit OR s.tp2_hit OR s.tp3_hit OR s.tp4_hit THEN 1 ELSE 0 END) > 0 THEN
        ROUND(COALESCE(SUM(CASE WHEN s.tp1_hit OR s.tp2_hit OR s.tp3_hit OR s.tp4_hit THEN 
          (regexp_match(s.pips_result, '([+-]?\d+\.?\d*)'))[1]::numeric 
        ELSE 0 END), 0) / 
    ...
    CASE 
      WHEN SUM(CASE WHEN s.sl_hit THEN 1 ELSE 0 END) > 0 THEN
        ROUND(ABS(COALESCE(SUM(CASE WHEN s.sl_hit THEN 
          (regexp_match(s.pips_result, '([+-]?\d+\.?\d*)'))[1]::numeric 
        ELSE 0 END), 0)) / 
