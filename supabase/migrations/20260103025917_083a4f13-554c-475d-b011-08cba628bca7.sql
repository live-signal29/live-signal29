...
-- Fix XAUUSD accuracy stats to correctly filter by closed date
...
RETURNS TABLE(
  ...
  monthly_wins bigint, 
  monthly_losses bigint, 
  monthly_accuracy numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH xauusd_closed AS (
    SELECT 
      ...
      s.sl_hit,
      s.pips_result,
      (s.updated_at AT TIME ZONE 'UTC')::date AS closed_date,
      CASE 
        WHEN COALESCE(s.tp1_hit, false) OR COALESCE(s.tp2_hit, false) OR COALESCE(s.tp3_hit, false) OR COALESCE(s.tp4_hit, false) THEN true 
        ELSE false 
      END AS is_win,
      CASE 
        WHEN COALESCE(s.sl_hit, false) AND NOT (COALESCE(s.tp1_hit, false) OR COALESCE(s.tp2_hit, false) OR COALESCE(s.tp3_hit, false) OR COALESCE(s.tp4_hit, false)) THEN true 
        ELSE false 
      END AS is_loss
    FROM signals s
    WHERE UPPER(s.category) IN ('COMMODITIES', 'COMMODITY')
      AND (UPPER(s.pair) LIKE '%XAU%' OR UPPER(s.pair) LIKE '%GOLD%')
      AND LOWER(s.signal_status) = 'close'
      AND (COALESCE(s.tp1_hit, false) OR COALESCE(s.tp2_hit, false) OR COALESCE(s.tp3_hit, false) OR COALESCE(s.tp4_hit, false) OR COALESCE(s.sl_hit, false))
  ),
  -- Today stats - only signals closed TODAY
  today_data AS (
    SELECT
      COUNT(*) AS t_total,
    ...
    FROM xauusd_closed
    WHERE closed_date = v_today
  ),
  -- Yesterday stats - only signals closed YESTERDAY  
  yesterday_data AS (
    SELECT
      COUNT(*) AS y_total,
  ...
  week_data AS (
    ...
    FROM xauusd_closed
    WHERE closed_date >= v_week_start AND closed_date <= v_week_end
  ),
  -- This month stats
  month_data AS (
    SELECT
    ...
    FROM xauusd_closed
    WHERE closed_date >= v_month_start AND closed_date <= v_month_end
  ),
  -- Overall stats
  overall_stats AS (
    SELECT
  ...
  SELECT 
    ...
    l.days_data,
    -- Today: return actual counts, NULL accuracy if no data
    t.t_total,
    t.t_wins,
    t.t_losses,
    CASE WHEN t.t_total > 0 AND (t.t_wins + t.t_losses) > 0
      THEN ROUND((t.t_wins::numeric / NULLIF(t.t_wins + t.t_losses, 0)) * 100, 1)
      ELSE NULL 
    END,
    -- Yesterday: return actual counts, NULL accuracy if no data
    y.y_total,
    y.y_wins,
    y.y_losses,
    CASE WHEN y.y_total > 0 AND (y.y_wins + y.y_losses) > 0
      THEN ROUND((y.y_wins::numeric / NULLIF(y.y_wins + y.y_losses, 0)) * 100, 1)
      ELSE NULL 
    END,
    -- Week: return actual counts, NULL accuracy if no data
    w.w_total,
    w.w_wins,
    w.w_losses,
    CASE WHEN w.w_total > 0 AND (w.w_wins + w.w_losses) > 0
      THEN ROUND((w.w_wins::numeric / NULLIF(w.w_wins + w.w_losses, 0)) * 100, 1)
      ELSE NULL 
    END,
    -- Month: return actual counts, NULL accuracy if no data
    m.m_total,
    m.m_wins,
    m.m_losses,
    CASE WHEN m.m_total > 0 AND (m.m_wins + m.m_losses) > 0
      THEN ROUND((m.m_wins::numeric / NULLIF(m.m_wins + m.m_losses, 0)) * 100, 1)
      ELSE NULL 
    END
