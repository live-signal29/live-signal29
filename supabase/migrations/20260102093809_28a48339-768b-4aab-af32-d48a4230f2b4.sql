...
-- Create updated function with date-wise breakdown using updated_at as closed_at
...
BEGIN
  ...
  weekend_stats AS (
    SELECT
      COUNT(*) AS we_total,
      COUNT(*) FILTER (WHERE is_win) AS we_wins,
      COUNT(*) FILTER (WHERE is_loss) AS we_losses,
      CASE 
        WHEN COUNT(*) FILTER (WHERE is_win OR is_loss) > 0 
        THEN ROUND((COUNT(*) FILTER (WHERE is_win)::numeric / NULLIF(COUNT(*) FILTER (WHERE is_win OR is_loss), 0)) * 100, 1)
        ELSE NULL 
      END AS we_accuracy
    FROM xauusd_closed
    WHERE closed_date >= week_start 
      AND closed_date <= week_end
      AND EXTRACT(DOW FROM closed_date) IN (0, 6)
  )
  SELECT 
    o.total_signals,
    o.total_wins,
