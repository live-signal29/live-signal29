...
interface SignalCardProps {
  ...
  livePrice?: number;
  showBanner?: boolean; // ✅ Banner show karne ke liye
}
...
const SignalCardNew = ({ 
  ...
  subscriptionStatus, 
  livePrice,
  showBanner = false // ✅ Default false
}: SignalCardProps) => {
  ...
  // ✅ Har 10 second me update (more accurate)
  useEffect(() => {
  ...
  // ✅ NEW Signal - Sirf 1 hour ke andar wale
  const signalDate = new Date(signal.created_at);
  const now = new Date();
  const diffMinutes = differenceInMinutes(now, signalDate);
  const isNewSignal = diffMinutes < 60 && lifecycle !== 'close';
  ...
  const isTodaySignal = isToday(new Date(signal.created_at));
  ...
  return (
    <div className="space-y-2">
      ...
      <div
        ...
        className="relative w-full rounded-[12px] bg-card border border-border/50 p-2.5 text-foreground shadow-sm hover:border-border hover:shadow-md transition-all duration-300 cursor-pointer overflow-hidden"
      >
        {/* ===== HEADER ===== */}
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {/* ✅ REAL TIME TIME DISPLAY - Dynamic */}
      ...
      {/* ✅ BANNERS - Only for Today's Signals */}
      {isTodaySignal && showBanner && (
