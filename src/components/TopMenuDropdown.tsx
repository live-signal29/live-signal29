...
export const TopMenuDropdown = () => {
  ...
  return (
    <DropdownMenu modal={false}>
      ...
      {/* Small, Fixed, Glass Menu */}
      <DropdownMenuContent 
        align="end" 
        className="w-[150px] rounded-xl bg-background/80 backdrop-blur-xl border border-border/50 shadow-xl shadow-black/5 p-1.5"
        sideOffset={8}
        // Disable scroll blocking taaki "scroll up se refresh na ho"
        onCloseAutoFocus={(e) => e.preventDefault()}
      >
        {/* Premium Items with Hover Effect */}
        <DropdownMenuItem 
          onClick={handleShare} 
          className="cursor-pointer rounded-lg px-2.5 py-1.5 text-[12px] transition-all duration-200 hover:bg-primary/10 hover:text-primary hover:scale-[1.02]"
        >
        ...
        <DropdownMenuItem 
          ...
          className="cursor-pointer rounded-lg px-2.5 py-1.5 text-[12px] text-destructive transition-all duration-200 hover:bg-destructive/10 hover:text-destructive hover:scale-[1.02]"
        >
