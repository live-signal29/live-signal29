...
import { Lock, PlayCircle, Crown, Loader2, Sparkles, Zap, CheckCircle2 } from "lucide-react";
...
export const SignalUnlockGate = ({
  ...
  // true for a brief moment right after the popup closes, so the
  // real card fades/zooms in instead of just popping into place
  ...
  // render the real card — with a quick fade/zoom-in if we just
  // unlocked it ourselves this session.
  if (!loading && isUnlocked) {
    return (
      <div className={revealing ? "animate-in fade-in-0 zoom-in-95 duration-500" : undefined}>
        {children}
      </div>
    );
  }
  ...
  const handleWatchAd = async () => {
    ...
    if (watched) {
      ...
      setWatchingAd(false);
      if (ok) {
        // Play the ding + show a quick checkmark inside the popup
        // before it closes and the real card fades in.
        playUnlockSound();
        ...
        <DialogContent className="max-w-[360px] rounded-3xl p-0 overflow-hidden gap-0">
          {watchingAd ? (
            <div className="flex flex-col items-center gap-4 px-6 py-10 text-center">
              {justUnlocked ? (
                <>
                  <CheckCircle2 className="h-[88px] w-[88px] text-emerald-500 animate-in zoom-in-50 duration-500" />
